...
# PyTest runner
# ---------------------------------------------------------------------------
async def run_pytest(path: str, subdir: str = "") -> dict:
    """Run pytest in `path/subdir`. Captures output + pass/fail summary."""
    target = str(Path(path) / subdir) if subdir else path
    if not Path(target).is_dir():
        return {"ok": False, "error": f"target dir not found: {target}"}
    code, out, err = await _run(
        ["python", "-m", "pytest", "-q", "--tb=short", "--maxfail=25", "-p", "no:cacheprovider"],
...
You will receive a failing PyTest report and optionally the contents of
relevant files. Emit ONE minimal unified diff (git-style) that fixes the
failures. No prose. No explanation. No markdown fences. Just the patch
starting with `diff --git`. Keep changes surgical. Do not rewrite unrelated code."""
...
async def _apply_patch(repo_dir: str, diff: str) -> tuple[bool, str]:
    patch_path = Path(repo_dir) / ".gh05t3_patch.diff"
    patch_path.write_text(diff, encoding="utf-8")
    code, out, err = await _run(
        ["git", "apply", "--whitespace=nowarn", ".gh05t3_patch.diff"],
        cwd=repo_dir,
    )
    try:
        patch_path.unlink()
        ...
        pass
    return code == 0, (out + err).strip()[:1000]


async def _read_touched_files(repo_dir: str, diff: str, max_chars: int = 6000) -> str:
    """Read files named in the diff and include a truncated snapshot."""
    files = set()
    for m in re.finditer(r"diff --git a/(\S+) b/\S+", diff):
        files.add(m.group(1))
    if not files:
        return ""
    out = []
...
async def run_task(
    ...
    nightly_chat,                 # injected from server.py
    ...
    # initial test run
    first = await run_pytest(str(work), subdir=subdir)
    ...
    for i in range(max_iterations):
        ...
        # Ask LLM for a patch
        sys_msg = PATCH_SYS
        snap = await _read_touched_files(str(work), first.get("tail", ""))
        user_msg = (
            f"Task: {task_description}\n\n"
            f"PyTest output:\n{first.get('tail','')[-3000:]}\n\n"
            f"(optional) file context:\n{snap[:6000]}\n\n"
            "Emit a unified diff only."
        )
        try:
            raw, tag = await nightly_chat(f"coder-{task_id}-{i}", sys_msg, user_msg)
        except Exception as e:  # noqa: BLE001
        ...
        if not diff or "diff --git" not in diff:
            attempts.append({"iter": i, "engine": tag, "applied": False,
                             "reason": "no diff emitted"})
            break
        applied, apply_msg = await _apply_patch(str(work), diff)
        if not applied:
            attempts.append({"iter": i, "engine": tag, "applied": False,
                             "reason": apply_msg[:300]})
            break
        # Re-run tests
        result = await run_pytest(str(work), subdir=subdir)
        attempts.append({
            "iter": i, "engine": tag, "applied": True,
            ...
            try:
                ...
                pr = r.create_pull(
                    ...
                    body=(
                        ...
                        f"**PyTest final:** passed={first.get('passed')} "
                        f"failed={first.get('failed')}\n\n"
